 ...
 *
 * Uses dated snapshots only. For each pool observation at time T, it looks
 * forward to the nearest snapshot at least N days later and measures the
 * observed TVL change. No future value is used to calculate the score at T.
 *
 * This is intentionally a baseline evaluator, not a model-tuning script.
 * Keep v1 weights frozen while we collect evidence.
 *
 * Run: npx tsx scripts/backtest-risk.ts
 */
...
function summarize(results: Result[]) {
  ...
  const quintiles = Array.from({ length: 5 }, (_, i) => {
    ...
    return {
      quintile: i + 1,
      n: slice.length,
      avgScore: slice.reduce((s, r) => s + r.score, 0) / slice.length,
      collapseRate: slice.filter((r) => r.collapsed).length / slice.length,
      disappearanceRate: slice.filter((r) => r.disappeared).length / slice.length,
...
function main() {
  ...
  const observations: Observation[] = snapshots.flatMap((s) =>
    s.pools.filter((p) => Number.isFinite(p.tvlUsd) && p.tvlUsd > 0).map((p) => ({ ...p, date: s.date })),
  )

  const output: Record<string, unknown> = {
    modelVersion: 'v1',
    collapseDefinition: `future TVL <= ${COLLAPSE_THRESHOLD * 100}% from baseline`,
    snapshots: snapshots.length,
    dateRange: [snapshots[0].date, snapshots[snapshots.length - 1].date],
    windows: {},
  }
  ...
  for (const days of WINDOWS) {
    ...
    for (const obs of observations) {
      const t = Date.parse(obs.date)
      const target = t + days * 86400_000
      ...
      if (!future) {
        results.push({
          ...obs,
          targetDate: futureSnapshot.date,
          futureTvlUsd: 0,
          tvlChange: -1,
          collapsed: true,
          disappeared: true,
      ...
      results.push({
        ...obs,
        targetDate: futureSnapshot.date,
        futureTvlUsd: future.tvlUsd,
        tvlChange,
        collapsed: tvlChange <= COLLAPSE_THRESHOLD,
        disappeared: false,
      })
    }
