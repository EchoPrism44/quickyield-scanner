...
import { alertTargetLabel } from '../../lib/alerts'
          ...
          {alerts.map((alert) => (
            <div key={alert.id} className="qy-table-row" style={{ gridTemplateColumns: '4fr 2fr 2fr 2fr 1fr', opacity: alert.enabled ? 1 : 0.55 }}>
              <div>
                ...
                <div className="qy-asset-meta">{alert.asset || 'Any asset'} · {alertTargetLabel(alert)}, Risk &lt;= {alert.maxRisk}</div>
              </div>
              <span className="qy-mono">{alert.asset}</span>
              <span className="qy-num bold">{alertTargetLabel(alert)}</span>
