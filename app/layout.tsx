import type { Metadata, Viewport } from 'next'
import { ClerkProvider } from '@clerk/nextjs'
import { Cinzel, Instrument_Sans, JetBrains_Mono, Newsreader } from 'next/font/google'
import './globals.css'

/**
 * Fonts are loaded through next/font (self-hosted, preloaded, no layout shift).
 * A plain CSS `@import url(fonts.googleapis...)` does NOT survive the Turbopack
 * build  -  it gets stripped, and every family silently falls back.
 *
 * Newsreader (serif) = editorial surfaces: marketing headlines + articles.
 * Instrument Sans = UI/data surfaces. JetBrains Mono = tickers and figures.
 */
const fontSerif = Newsreader({
  subsets: ['latin'],
  display: 'swap',
  style: ['normal', 'italic'],
  variable: '--font-serif-src',
})
const fontSans = Instrument_Sans({
  subsets: ['latin'],
  display: 'swap',
  variable: '--font-sans-src',
})
const fontMono = JetBrains_Mono({
  subsets: ['latin'],
  display: 'swap',
  variable: '--font-mono-src',
})
const fontWordmark = Cinzel({
  subsets: ['latin'],
  display: 'swap',
  weight: ['500'],
  variable: '--font-wordmark-src',
})

// Use one canonical public origin everywhere. Set NEXT_PUBLIC_SITE_URL to
// https://www.getlitmus.xyz in production; the explicit fallback keeps local
// and preview builds from silently generating a different canonical origin.
const siteUrl = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://www.getlitmus.xyz'

export const metadata: Metadata = {
  metadataBase: new URL(siteUrl),
  title: {
    default: 'Litmus  -  Yield research you can audit, not just trust',
    template: '%s | Litmus',
  },
  description:
    'Litmus grades every live onchain yield pool A–F under a published methodology and commits every grade to a public, timestamped ledger. Non-custodial research and alerts.',
  alternates: {
    canonical: '/',
  },
  icons: {
    icon: [
      { url: '/icon', type: 'image/png', sizes: '48x48' },
      { url: '/favicon.svg', type: 'image/svg+xml' },
    ],
    shortcut: '/icon',
    apple: '/apple-icon',
  },
  openGraph: {
    title: 'Litmus  -  Yield research you can audit, not just trust',
    description:
      'Every pool graded A–F under a published methodology. Every grade committed to a public, timestamped ledger before the outcome is known.',
    type: 'website',
    siteName: 'Litmus',
    url: siteUrl,
  },
  twitter: {
    card: 'summary_large_image',
    title: 'Litmus  -  Yield research you can audit, not just trust',
    description:
      'Every pool graded A–F under a published methodology. Every grade committed to a public, timestamped ledger before the outcome is known.',
  },
}

export const viewport: Viewport = {
  themeColor: '#07090d',
  colorScheme: 'dark',
}

const clerkAppearance = {
  variables: {
    colorPrimary: '#2f88ff',
    colorBackground: '#10151c',
    colorText: '#f5f6f8',
    colorTextSecondary: '#a6adba',
    colorInputBackground: '#0a0d12',
    colorInputText: '#f5f6f8',
    colorNeutral: '#f5f6f8',
    borderRadius: '10px',
    fontFamily: 'var(--font-sans, ui-sans-serif, system-ui, sans-serif)',
  },
  elements: {
    card: { backgroundColor: '#10151c', border: '1px solid rgba(255,255,255,0.08)', boxShadow: '0 16px 48px -16px rgba(0,0,0,0.55)' },
    headerTitle: { color: '#f5f6f8' },
    headerSubtitle: { color: '#a6adba' },
    socialButtonsBlockButton: { borderColor: 'rgba(255,255,255,0.12)' },
    formButtonPrimary: { backgroundColor: '#2f88ff', color: '#ffffff', textTransform: 'none' as const, fontWeight: 600 },
    footerActionLink: { color: '#2f88ff' },
  },
}

const clerkPublishableKey = process.env.NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY
const clerkEnabled = typeof clerkPublishableKey === 'string' && clerkPublishableKey.startsWith('pk_')

const structuredData = {
  '@context': 'https://schema.org',
  '@graph': [
    {
      '@type': 'Organization',
      '@id': `${siteUrl}/#organization`,
      name: 'Litmus',
      url: siteUrl,
      logo: {
        '@type': 'ImageObject',
        url: `${siteUrl}/apple-icon`,
        width: 180,
        height: 180,
      },
      description:
        'Litmus grades every live onchain yield pool A–F under a published methodology and publishes the record every week.',
    },
    {
      '@type': 'WebSite',
      '@id': `${siteUrl}/#website`,
      url: siteUrl,
      name: 'Litmus',
      publisher: { '@id': `${siteUrl}/#organization` },
    },
  ],
}

export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  const html = (
    <html lang="en" className={`${fontSerif.variable} ${fontSans.variable} ${fontMono.variable} ${fontWordmark.variable}`}>
      <head>
        <script
          type="application/ld+json"
          dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData) }}
        />
      </head>
      <body>{children}</body>
    </html>
  )
  if (!clerkEnabled) return html
  return <ClerkProvider appearance={clerkAppearance}>{html}</ClerkProvider>
}